...
Revision 1.02 - Added Log Comment  2003/03/24 19:37:42  ckeith
  ...
  <xsl:template name="marcRecord">
    ...
    <xsl:for-each select="marc:datafield[@tag=767]|marc:datafield[@tag=775]">
...
/>
  ...
  <xsl:template match="processing-instruction('family')">
    <namePart type="family">
      <xsl:value-of select="."/>
    </namePart>
  </xsl:template>

  <xsl:template match="processing-instruction('given')">
    <namePart type="given">
      <xsl:value-of select="."/>
    </namePart>
  </xsl:template>
  ...
  <xsl:template name="nameABCDQ">
    <xsl:choose>
      <xsl:when test="processing-instruction('family') and
		      processing-instruction('given')">
	<xsl:apply-templates select="processing-instruction('family')"/>
	<xsl:apply-templates select="processing-instruction('given')"/>
      </xsl:when>
      <xsl:otherwise>
	<namePart>
	  <xsl:call-template name="chopPunctuation">
	    <xsl:with-param name="chopString">
	      <xsl:call-template name="subfieldSelect">
		<xsl:with-param name="codes">ahq</xsl:with-param>
	      </xsl:call-template>
	    </xsl:with-param>
	    <xsl:with-param name="punctuation">
	      <xsl:text>:,;/ </xsl:text>
